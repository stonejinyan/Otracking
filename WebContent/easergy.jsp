...
<body>
	...
	<script type="text/javascript">
		...
		window.onload = function() {
			...
			securityCode.oninput = function() {
				if(modata.value == null || modata.value == "" || modata.value == undefined){
					setInputClear();
					modata.focus();
					log.innerText = new Date().toLocaleString().split(" ")[1]+":" + "安装位置码不能为空！" + '\n'
					+ log.value;
				}else if(securityCode.value == null || securityCode.value == "" || securityCode.value == undefined){
					setInputClear();
					modata.focus();
					log.innerText = new Date().toLocaleString().split(" ")[1]+":" + "温控传感器码不能为空！" + '\n'
					+ log.value;
				}else {
				sentSecurityCode(modata.value,securityCode.value);
				setInputClear();
				modata.focus();
				}
			}
		...
		function sentSecurityCode(mo_name,securityCode){
				var strArray = mo_name.split('?');
				//alert(strArray[0]);
				//alert(strArray[1]);
				//alert(securityCode);
			$
			.ajax({
				url : 'SentEasergyCodeAction?aop='+strArray[0]+'&position='+strArray[1]+'&easergyCode='+securityCode,
				type : 'GET',
				success : function(data) {
					//alert(data);
					if(data == 1){
						log.innerText = new Date().toLocaleString().split(" ")[1]+":" +mo_name + "录入传感器码" + securityCode + '\n'
						+ log.value;
					}else{
						log.innerText = new Date().toLocaleString().split(" ")[1]+":" + "安装位置不存在！" + '\n'
						+ log.value;
					}
				}
			});
